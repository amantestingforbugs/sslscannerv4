...
def test_project_host_upload_accepts_csv_file_payload(tmp_path, monkeypatch):
    """The project upload dropzone advertises CSV support, so CSV rows should import."""
    sys.path.insert(0, str(ROOT))
    from flask import Flask
    ...
    monkeypatch.setattr(database, "DB_PATH", tmp_path / "project_hosts_csv.sqlite3")
    monkeypatch.setattr(database, "_local", __import__("threading").local())
    database.init_db()

    project = database.project_create("csv-hosts")
    app = Flask(__name__)
    app.register_blueprint(routes.api)
    client = app.test_client()
    ...
    resp = client.post(
        ...
        data={"file": (BytesIO(b"example.com,https://api.example.com\n"), "hosts.csv")},
    )
...
def test_project_host_upload_accepts_raw_text_payload(tmp_path, monkeypatch):
    """Non-browser clients can upload the same host list as plain text."""
    sys.path.insert(0, str(ROOT))
    from flask import Flask
    import db.database as database
    import api.routes as routes

    monkeypatch.setattr(database, "DB_PATH", tmp_path / "project_hosts_raw.sqlite3")
    monkeypatch.setattr(database, "_local", __import__("threading").local())
    database.init_db()

    project = database.project_create("raw-hosts")
    app = Flask(__name__)
    app.register_blueprint(routes.api)
    client = app.test_client()
...
def test_project_delete_route_removes_project_scoped_data(tmp_path, monkeypatch):
    sys.path.insert(0, str(ROOT))
    from flask import Flask
    import db.database as database
    import api.routes as routes

    monkeypatch.setattr(database, "DB_PATH", tmp_path / "project_delete.sqlite3")
    monkeypatch.setattr(database, "_local", __import__("threading").local())
    database.init_db()

    project = database.project_create("delete-me")
    pid = project["id"]
    database.project_save_hosts(pid, ["www.example.com"])
    scan = database.scan_create(pid, 1, "manual")
    database.results_batch_save(scan["id"], pid, [{"hostname": "www.example.com", "cn": "www.example.com"}])
    job_id = database.subfinder_job_create(pid, "example.com", by="manual")
    database.subfinder_hosts_add_batch(pid, ["api.example.com"])
    database.subfinder_new_discoveries_add_batch(job_id, pid, ["api.example.com"])
    database.subfinder_httpx_results_upsert_batch(pid, job_id, [{"hostname": "api.example.com", "is_active": True}])
    database.subfinder_raw_result_add(job_id, pid, "example.com", "subfinder -d example.com")
    database.job_create("nuclei_scan", id="delete-job", project_id=pid, status="running")
    database.job_event_append("delete-job", "started", {"project_id": pid})
    ...
    assert resp.status_code == 200
    assert resp.json == {"ok": True, "data": {"deleted": True, "id": pid}}
    assert database.project_get(pid) is None
    for table in (
        "projects",
        "scans",
        "results",
        ...
        if table in {"job_events", "job_controls"}:
            count = database.x(f"SELECT COUNT(*) c FROM {table} WHERE job_id='delete-job'").fetchone()["c"]
        elif table == "projects":
            count = database.x(f"SELECT COUNT(*) c FROM {table} WHERE id=?", (pid,)).fetchone()["c"]
        else:
            count = database.x(f"SELECT COUNT(*) c FROM {table} WHERE project_id=?", (pid,)).fetchone()["c"]
        assert count == 0, table


def test_project_delete_route_reports_missing_project(tmp_path, monkeypatch):
    sys.path.insert(0, str(ROOT))
    from flask import Flask
    import db.database as database
    import api.routes as routes

    monkeypatch.setattr(database, "DB_PATH", tmp_path / "project_delete_missing.sqlite3")
    monkeypatch.setattr(database, "_local", __import__("threading").local())
    database.init_db()
    ...
    resp = client.delete("/api/projects/missing")

    assert resp.status_code == 404
    assert resp.json == {"ok": False, "error": "Project not found"}
