...
def test_project_delete_tolerates_legacy_db_missing_new_cleanup_tables(tmp_path, monkeypatch):
    sys.path.insert(0, str(ROOT))
    import db.database as database

    monkeypatch.setattr(database, "DB_PATH", tmp_path / "project_delete_legacy.sqlite3")
    monkeypatch.setattr(database, "_local", __import__("threading").local())
    database.init_db()

    project = database.project_create("legacy-delete")
    pid = project["id"]
    database.x("DROP TABLE subfinder_httpx_results")
    database.x("DROP TABLE asset_findings")
    database.commit()

    assert database.project_delete(pid) is True
    assert database.project_get(pid) is None
